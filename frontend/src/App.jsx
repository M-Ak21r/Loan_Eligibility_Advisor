...
export default function App() {
  ...
  const handleSubmit = async (payload) => {
    ...
    try {
      ...
      setResult(data)
      // Fire an asynchronous assistant request with the application payload
      ;(async () => {
        try {
          const chatPrompt = 'Please explain these prediction results in plain language and advise next steps.'
          const chatRes = await fetch(`${API_BASE}/api/v1/chat`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
          ...
          if (!chatRes.ok) {
            ...
            throw new Error(detail?.detail ?? `Server error ${chatRes.status}`)
          }

          const chatData = await chatRes.json()
          // Inject assistant message into ChatWidget via a window event
          window.dispatchEvent(new CustomEvent('assistantMessage', { detail: { text: chatData.response } }))
        } catch (err) {
          window.dispatchEvent(new CustomEvent('assistantMessage', { detail: { text: `Assistant error: ${err.message ?? 'unknown'}` } }))
        }
      })()
    } catch (err) {
